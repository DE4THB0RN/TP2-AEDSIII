...
public class ArquivoLivro {
  ...
  public int create(Livro liv) throws Exception {
    arquivo.seek(0);
  ...
  public boolean update(Livro novoliv) throws Exception {
    Livro liv = new Livro();
    byte lapide;
    short tam, tam2;
    byte[] ba, ba2;
    ...
    while (arquivo.getFilePointer() < arquivo.length()) {
      ...
      if (lapide != '*') {
        ...
        if (liv.getID() == novoliv.getID()) {

          ba2 = novoliv.toByteArray();
  ...
  // Recebe um liveto vazio para auxiliar na reorganização
  public void reorganizar() throws Exception {
    ...
    // ---------------------------------------------------------------------
    List<Livro> registrosOrdenados = new ArrayList<>();
    ...
    try {
      ...
      while (true) {
        ...
        if (lapide == ' ') {
          registrosOrdenados.add((Livro) r1.clone());
          contador++;
        }
        if (contador == tamanhoBlocoMemoria) {
          ...
          for (Livro r : registrosOrdenados) {
            dados = r.toByteArray();
